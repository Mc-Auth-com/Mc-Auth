...
app.use(expressSession({
  ...
  rolling: true,
  unset: 'destroy',
  cookie: { secure: cfg.cookies.secure, httpOnly: true, maxAge: 60 * 24 * 60 * 60 * 1000 /* 60d */ }
}));
...
app.use((req, res, next) => {
  ...
  if (req.url.match(/^\/[a-z]{2}(\/|$)/gi)) {
    ...
    if (getLocalization().isAvailable(langKey)) {
      ...
      res.cookie('lang', res.locals.lang, { httpOnly: true, path: '/', secure: cfg.cookies.secure, maxAge: 12 * 30 * 24 * 60 * 60 * 1000 /* 12mo */ });
